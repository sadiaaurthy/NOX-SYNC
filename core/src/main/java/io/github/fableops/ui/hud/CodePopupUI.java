 ...
 *
 * Styled to match the JavaFX launcher's palette (launcher.css) — dark panel, orange
 * accent, HUD-bracket corners — so the launcher and in-game HUD read as one game.
 * ShapeRenderer can't do the launcher's clip-path stepped corners, so this uses
 * bracket-line accents instead of a literal notch.
 */
public class CodePopupUI {
    ...
    private static final float WIDTH_FRACTION = 0.4f;
    private static final float HEIGHT_FRACTION = 0.65f;
    private static final float PADDING = 26f;
    private static final float BRACKET_SIZE = 22f;

    private static final Color COLOR_BG      = new Color(0.04f, 0.043f, 0.047f, 0.96f);
    ...
    public void render(ShapeRenderer shape, SpriteBatch batch) {
        ...
        shape.rect(panelX, panelY, panelW, panelH);
        // HUD bracket accents at opposite corners, echoing the launcher panel's frame.
        shape.setColor(COLOR_ORANGE);
        shape.line(panelX, panelY + panelH, panelX + BRACKET_SIZE, panelY + panelH);
        shape.line(panelX, panelY + panelH, panelX, panelY + panelH - BRACKET_SIZE);
        shape.line(panelX + panelW, panelY, panelX + panelW - BRACKET_SIZE, panelY);
        shape.line(panelX + panelW, panelY, panelX + panelW, panelY + BRACKET_SIZE);
        shape.end();
